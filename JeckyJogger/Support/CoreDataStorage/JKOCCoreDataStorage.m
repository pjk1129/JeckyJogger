...
#ifdef DEBUG
#define JKLog(xx, ...)  NSLog(@"%s(%d): " xx, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define JKLog(xx, ...)  ((void)0)
#endif

@implementation JKOCCoreDataStorage
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        JKLog(@"%@: Creating managedObjectModel (%@)", [self class], momName);
        ...
        {
            JKLog(@"%@: Couldn't find managedObjectModel file - %@", [self class], momName);
        }
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        JKLog(@"%@: Creating persistentStoreCoordinator", [self class]);
        ...
        if (databaseFileName)
        {
            ...
            {
                JKLog(@"%@: Error creating persistentStoreCoordinator - Nil persistentStoreDirectory",
                            [self class]);
            }
        }
        else
...
- (NSManagedObjectContext *)managedObjectContext
{
    ...
    if (coordinator)
    {
        JKLog(@"%@: Creating managedObjectContext", [self class]);
...
- (NSManagedObjectContext *)mainThreadManagedObjectContext
{
    ...
    if (coordinator)
    {
        JKLog(@"%@: Creating mainThreadManagedObjectContext", [self class]);
...
- (void)save
{
    ...
    {
        JKLog(@"%@: Error saving - %@ %@", [self class], error, [error userInfo]);
...
- (void)maybeSave:(int32_t)currentPendingRequests
{
    ...
    if ([[self managedObjectContext] hasChanges])
    {
        ...
        {
            JKLog(@"%@: Triggering save (pendingRequests=%i)", [self class], currentPendingRequests);
        ...
        else
        {
            ...
            {
                JKLog(@"%@: Triggering save (unsavedCount=%lu)", [self class], (unsigned long)unsavedCount);
